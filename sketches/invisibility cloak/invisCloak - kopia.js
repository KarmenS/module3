...
console.log(centroidSpeed);
    ...
    for (let i = 0; i < bubbles.length; i++) {
      ...
      if (distanceCentroidAndBubble < 60) {
        bubbles[i].velocityX =
          Math.atan(
            (bubbles[i].y - nose.position.y) / (bubbles[i].x - nose.position.x)
          ) * nose.speed.absoluteSpeed /3;
        bubbles[i].velocityY =
          Math.asin(
            (bubbles[i].y - nose.position.y) / (bubbles[i].x - nose.position.x)
          ) * nose.speed.absoluteSpeed /3;
...
function drawPart(part) {
  ctx.beginPath();
  ctx.arc(part.position.x, part.position.y, 100, 0, 2 * Math.PI);
  ctx.fillStyle = "pink";
  ctx.fill();
}
